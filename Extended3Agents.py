...
class SchellingAgent(Agent):
    ...
    def __init__(self, unique_id, model, agent_group):
        ...
        base_income = {1: 4, 2: 7, 3: 10}[agent_group]
    ...
    def step(self):
        ...
        if unhappy_move or random_move:
            ...
            for district in (0, 1, 2):
                ...
                rent = self.model.rents[district]
                consumption = self.income - min(rent, self.income)

                E_payoffs_per_district[district] = (consumption**self.model.alpha) * (E_payoff**(1-self.model.alpha))
...
class SchellingModel(Model):
    ...
    def __init__(self, width, height, density, p_random, pay_c, pay_m, max_tenure, u_threshold, alpha, population_distribution, seed=None):
        super().__init__(seed)
        self.width = width
        self.height = height
    ...
    def step(self):
        ...
        # To determine the rents for the next step, we create pairs of (agentID, WTP)
        demands = {0: [], 1: [], 2: []}
        for agent in self.schedule.agents:
            expA, expB, expC = np.exp(agent.U_A), np.exp(agent.U_B), np.exp(agent.U_C)
            total = expA + expB + expC
            Probs = {'a': expA/total, 'b': expB/total, 'c':expC/total}

            for district in (0, 1, 2):
                E_payoff = (Probs['a'] * (self.pay_c*mu[district]['a'] + self.pay_m*(1-mu[district]['a'])) + 
                            Probs['b'] * (self.pay_c*mu[district]['b'] + self.pay_m*(1-mu[district]['b'])) + 
                            Probs['c'] * (self.pay_c*mu[district]['c'] + self.pay_m*(1-mu[district]['c'])))
                
                WTP = (agent.income**self.alpha) * (E_payoff**(1-self.alpha))
                demands[district].append((agent, WTP))

        # Using these pairs we determine the rents
        self.rents = {}
        for district, bids in demands.items():
            bids.sort(key=lambda x: x[1], reverse=True)
            supply = (self.width // 3) * self.height

            if len(bids) >= supply:
                rent = bids[supply - 1][1]
            else:
                rent = bids[-1][1] if bids else 0

            self.rents[district] = rent
...
u_threshold = 7
alpha = 0.5
population_distribution = [1/3, 1/3, 1/3]
base_incomes = [4, 7, 10]
radius = 1
steps = 1000

# Initialize and run model
model = SchellingModel(width, height, density, p_random, pay_c, pay_m, max_tenure, u_threshold, alpha, population_distribution)
snapshots = []
happiness_fractions = []
